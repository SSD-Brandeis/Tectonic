#!/usr/bin/env python3
"""Plot db_bench-compatible vs Tectonic workload generation metrics."""
...
TOOLS = [
    ("db_bench_wrapper", r"db\_bench wrapper", {"facecolor": "white", "edgecolor": "0.25", "hatch": "///"}),
...
def save_legend(base: Path) -> None:
    ...
    fig = plt.figure(figsize=(4.6, 0.6))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot db_bench workload-generation comparison results.")
    ...
    plot_metric(
        ...
        r"cpu time (s / M ops)",
        "db_bench_generation_cpu_time",
    )
    plot_metric(
        data,
        "duration_s",
        r"workload generation time (s)",
        "db_bench_generation_wall_time",
    )
    plot_metric(
        data,
        "peak_vmhwm_kb",
        r"peak memory footprint (MB)",
        "db_bench_generation_peak_memory",
        scale=1.0 / 1024.0,
    )
    save_legend(OUT_DIR / "db_bench_generation")
    print(f"saved generation plots to {OUT_DIR}")
